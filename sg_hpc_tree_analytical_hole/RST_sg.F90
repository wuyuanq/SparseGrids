...
contains
    ...
    recursive subroutine sgSubInterpo(z, pPoint, estArray)
        ...
        if(pPoint%isInSG) then
            ...
            if(isInRange) then
                do i = 1, DIM
                    if((z(i)<pPoint%lend(i)).or.(z(i)>pPoint%rend(i))) then
                        isInRange = .false.
                        exit
                    else
                        ratio = ratio * ((pPoint%rend(i)-pPoint%lend(i))/2.D0 - &!
                            abs(z(i)-pPoint%coordinate(i)))/ ((pPoint%rend(i)-pPoint%lend(i))/2.D0)
                    end if
                end do
            end if
        ...
        do i = 1, DIM
            ...
            if(associated(childPoint).and.((childPoint%isInSG.and.(z(i)>=childPoint%lend(i)).and. &!
                (z(i)<=childPoint%rend(i))).or.(.not.childPoint%isInSG))) then
                call sgSubInterpo(z, childPoint, estArray)
            else
    ...
    subroutine sgInterpo(z, estArray)
        ...
        do while(associated(pilp))
            call sgSubInterpo(z, pilp%pp, estArray)
            pilp => pilp%next
        end do
    ...
    subroutine sgHierarchy(thisNode)
        ...
        do i = 1, DIM
            z(i) = pointLocate(thisNode%myID, i)
        end do
        isInRange = .true.

        isRea = .true.
        if(isInRange) then
            xiL = cos((z(4)*TWOPI-z(5)*TWOPI)/2.D0)**2.D0*cos(z(2)*TWOPI)* &!
                (z(3)*TWOPI*sin(z(6)*TWOPI)+z(1)*TWOPI)
xiL = sin(z(1)*TWOPI)+sin(z(2)*TWOPI)+sin(z(3)*TWOPI)+cos(z(4)*TWOPI)+cos(z(5)*TWOPI)+z(6)*TWOPI
xiL = sin(z(1))+sin(z(2))+sin(z(3))+cos(z(4))+cos(z(5))+cos(z(6))
            x = 0.D0
            y = 0.D0
            xiG = 0.D0
            rhoL = 0.D0
    ...
    recursive subroutine sgTest(dIndex)
        ...
        if(dIndex == DIM+1) then
            ...
            do i = 1, DIM
                z(i) = sgTestArray(i)*1.D0/(NUMSAMPLE+1)
            end do
            isInRange = .true.
